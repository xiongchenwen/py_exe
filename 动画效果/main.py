    ...
    def Drawing_1(self):
        ...
        for i in range(0, 181):
            if angle < 180:
                print('angle', angle)
                min_angle = angle
                boundary_point, route_point, top_list, low_list, point = algorithm.polyScan_Draw(point_1, point_offset,
                                                                                         path_width, min_angle)
                # 画车辆边界线与车辆路线
        ...
        # plt.axes.canvas.draw()
        # plt.axes.canvas.flush_events()

        # self.horizontalLayout.canvas.draw()
        # self.horizontalLayout.canvas.flush_events()
        # time.sleep(0.5)
